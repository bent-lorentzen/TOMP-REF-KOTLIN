...
interface SupportApi {
    ...
    @ApiResponses(
        value = [ApiResponse(
            ...
            content = Content(mediaType = "application/json", array = ArraySchema(schema = Schema(implementation = SupportStatus::class)))
        ), ApiResponse(
            responseCode = "400",
    ...
    @ApiResponses(
        value = [ApiResponse(
            ...
            content = Content(mediaType = "application/json", schema = Schema(implementation = SupportStatus::class))
        ), ApiResponse(
            responseCode = "400",
            description = "Bad request. See https://github.com/TOMP-WG/TOMP-API/wiki/Error-handling-in-TOMP for further explanation of error code.",
            content = Content(mediaType = "application/json", schema = Schema(implementation = Error::class))
    ...
    @RequestMapping(value = ["/support/"], produces = ["application/json"], consumes = ["application/json"], method = [RequestMethod.POST])
    fun supportPost(
        @Parameter(
            `in` = ParameterIn.HEADER,
            description = "A list of the languages/localizations the user would like to see the results in. For user privacy and ease of use on the TO side, this list should be kept as short as possible, ideally just one language tag from the list in operator/information",
